...
exports.connect = function(host){
  ...
  socket.on('message', function(obj){
    ...
    if(obj.type === 'CLIENT_VARS'){
      ...
      assert((obj.data.newRev - 1) == padState.baseRev, 'wrong incoming revision :' + obj.data.newRev + '/' + padState.baseRev);
  ...
  // Function to apply an attribute to a pad contents
  ee.apply = function(){
    // exports.makeSplice = function (oldFullText, spliceStart, numRemoved, newText, optNewTextAPairs, pool) {
    var newChangeset = Changeset.makeSplice(padState.atext.text, padState.atext.text.length, 0, "HELLOWORLD", [4,true], padState.apool);
    ...
    // Create a blank attribute pool for the wire
    var wireApool = new AttributePool().toJsonable(); // TODO?
  ...
  // Both server and client edited the document. We need to update local change with remote changes
  // and visa versa
  function transformX(client, server) {
    var _c = Changeset.follow(server.changeset, client.changeset, false, padState.pool);
    var _s = Changeset.follow(client.changeset, server.changeset, true, padState.pool);
    client.changeset = _c;
    server.changeset = _s;
  }

  // sends a message to the server. Can be used without parameters to try sending outgoing message
  function sendMessage(optMsg) {
    // console.log("sending message: ", optMsg && (optMsg.changeset + '[' + optMsg.baseRev + ']'),
    //   '; inflight: ' + (padState.inFlight && (padState.inFlight.changeset + '[' + padState.inFlight.baseRev + ']')), 
    //   '; outgoing: ' + (padState.outgoing && (padState.outgoing.changeset + '[' + padState.outgoing.baseRev + ']')) 
    //   );
    if(optMsg) {
      if(padState.outgoing) {
        assert(optMsg.baseRev == padState.outgoing.baseRev, 'should append to the same document version');

        padState.outgoing.changeset = Changeset.compose(padState.outgoing.changeset, optMsg.changeset, padState.apool);
      } else {
