...
exports.connect = function(host){
  ...
  socket.on('message', function(obj){
    ...
    if(obj.type === 'CLIENT_VARS'){
      ...
      padState.pendingChange = Changeset.follow(padState.pendingChange, c, true); 
...
function drainPending(padState, socket){
  if(padState.inFlight == null && !Changeset.isIdentity(padState.pendingChange)) { // in flight is basically if we're already sending messages?
    ...
    // I don't understand what this is doing.. - creates identity changeset -- Which is what?
    // which is changeset about nothing
    ...
    console.log("sent", padState.inFlight);
